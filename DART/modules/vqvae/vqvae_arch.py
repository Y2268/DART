...
import DART.distributed as dist_fn
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        if self.training and self.entropy_loss_ratio > 0:
            ...
            # 计算平均概率分布（每个codebook条目被选中的平均概率）
            avg_probs = torch.mean(probs, dim=0)
            
            # 计算平均熵 - 鼓励使用多样化的codebook条目
            ...
            # 打印调试信息
            if torch.rand(1)[0] < 0.01:  # 1%概率打印信息
                # 计算codebook利用率
                unique_indices = torch.unique(min_encoding_indices)
                usage_percent = 100.0 * unique_indices.numel() / self.n_e
                print(f"Codebook利用率: {unique_indices.numel()}/{self.n_e} ({usage_percent:.2f}%)")
                print(f"熵损失: {entropy_loss.item():.4f}, 平均熵: {avg_entropy.item():.4f}, 样本熵: {sample_entropy.item():.4f}")
    ...
    def get_codebook_entry(self, indices, shape): #解码
        # shape specifying (batch, height, width, channel)
        # unique_indices = torch.unique(indices)
        # num_unique = len(unique_indices)
        # total_indices = indices.numel()
        # usage_percent = 100.0 * num_unique / self.n_e
        # print(f"索引统计: 不同索引数={num_unique}/{self.n_e} ({usage_percent:.2f}%), 总位置数={total_indices}")
        # print(f"索引值范围: [{indices.min().item()}, {indices.max().item()}]")
        
        min_encodings = torch.zeros(indices.shape[0], self.n_e).to(indices)
        min_encodings.scatter_(1, indices[:,None], 1)
            # 添加：检查embedding权重
        # print(f"Embedding权重统计: 形状={self.embedding.weight.shape}, "
        #     f"最小值={self.embedding.weight.min().item():.4f}, "
        #     f"最大值={self.embedding.weight.max().item():.4f}, "
        #     f"均值={self.embedding.weight.mean().item():.4f}, "
        #     f"标准差={self.embedding.weight.std().item():.4f}")
        # # 检查几个词向量示例
        # num_samples = min(3, self.embedding.weight.shape[0])
        # sample_dim = min(3, self.embedding.weight.shape[1])
        # print("Embedding词向量样本:")
        # for i in range(num_samples):
        #     vec_sample = self.embedding.weight[i, :sample_dim].tolist()
        #     print(f"  向量{i}: {vec_sample}")
        #     # 检查one-hot编码
        # print(f"One-hot编码形状: {min_encodings.shape}")
        # print(f"每行sum: {min_encodings.sum(1)[:5]}")  # 确认每行只有一个1
        ...
        # if z_q.numel() > 0:
            # 基本统计信息
            # print(f"特征向量统计: 形状={z_q.shape}, "
            #     f"最小值={z_q.min().item():.4f}, "
            #     f"最大值={z_q.max().item():.4f}, "
            #     f"均值={z_q.mean().item():.4f}, "
            #     f"标准差={z_q.std().item():.4f}")
            
            # # 可视化前几个位置的部分向量值(避免打印太多)
            # if z_q.shape[0] > 0:
            #     sample_idx = min(3, z_q.shape[0])  # 最多显示5个样本
            #     sample_dim = min(3, z_q.shape[1] if len(z_q.shape) > 1 else 0)  # 每个样本最多显示5个维度
                
            #     print("样本特征向量片段:")
            #     for i in range(sample_idx):
            #         if len(z_q.shape) == 2:  # 如果是二维张量
            #             vec_sample = z_q[i, :sample_dim].tolist()
            #             print(f"  样本{i}: {vec_sample}")
            #         else:
            #             print(f"  样本{i}: 形状过于复杂，无法显示")
...
class MultiHeadEncoder(nn.Module):
    def __init__(self, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks=2,
        ...
        for i_level in range(self.num_resolutions):
            ...
            down.block = block
            down.attn = attn
            if i_level != self.num_resolutions-1: # 5 最后一次不下采样
    ...
    def forward(self, x):
        ...
        # downsampling
        h = self.conv_in(x) #将输入图像512x512x3转换为512x512xch
        hs['in'] = h
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
            ...
            if i_level != self.num_resolutions-1: # 5 最后一次不下采样，最终输出16x16x512
                # hs.append(h)
        ...
        h = self.conv_out(h)
        # hs.append(h)
        hs['out'] = h  #hs['out']是最终的输出，通道数是z_channels
