...
class FFHQDegradationDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return_dict = {
                'lq': img_lq,
                'gt': img_gt, # for LQ dictionary training
                # 'gt': img_gt,
                'gt_path': gt_path,
            }
        if self.crop_components:
    ...
    # For generating degraded images
    def getitem_degraded(self, index):
        if self.file_client is None:
            self.file_client = FileClient(self.io_backend_opt.pop('type'), **self.io_backend_opt)
...
if __name__=='__main__':
    # pdb.set_trace()
    # base='configs/data_generation.yaml'
    base='configs/data_generation_v2.yaml'
    ...
    for i in tqdm(range(100)):
        ...
        # cv2.imwrite('/ssd2/yuju/RestoreFormer/distribution/HQ_img/'+name+'_gt.png', gt)
        lq = tensor2img(sample['lq'])
        # cv2.imwrite('/ssd2/yuju/RestoreFormer/data/self_celeba_512_v2/'+name+'.png', lq)
        cv2.imwrite('/ssd2/yuju/RestoreFormer/distribution/LQ_img2/'+name+'.png', lq)