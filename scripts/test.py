...
from main_DART import instantiate_from_config, DataModuleFromConfig
...
def restoration(model,
    ...
    for idx, cropped_face in enumerate(face_helper.cropped_faces):
        ...
        #cropped_face_t = cropped_face_t.unsqueeze(0).to('xpu')
        # output = [dec, diff, info, hs, h, quant]

        try:
            with torch.no_grad():
                
                output = model(cropped_face_t, filenames=[basename])
                restored_face = tensor2img(output[0].squeeze(0), rgb2bgr=True, min_max=(-1, 1))

        except RuntimeError as error:
            print(f'\tFailed inference for DART: {error}.')
